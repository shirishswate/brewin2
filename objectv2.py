...
class ObjectDef:
    ...
    def call_method(self, method_name, actual_params, line_num_of_caller):
        ...
        for formal, actual in zip(method_info.formal_params, actual_params):
            # check for int
            if (method_info.get_formal_params()[formal] == 'int' and actual.type() != Type.INT):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
            elif (method_info.get_formal_params()[formal] != 'int' and actual.type() == Type.INT):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
                
            # check for bool
            elif (method_info.get_formal_params()[formal] == 'bool' and actual.type() != Type.BOOL):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
            elif (method_info.get_formal_params()[formal] != 'bool' and actual.type() == Type.BOOL):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
                
            # check for string
            elif (method_info.get_formal_params()[formal] == 'string' and actual.type() != Type.STRING):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
            elif (method_info.get_formal_params()[formal] != 'string' and actual.type() == Type.STRING):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
                
            # check for class
            elif (method_info.get_formal_params()[formal] in self.interpreter.get_classes() and actual.type() != Type.CLASS):
                self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect parameter type ")
            
            # check if class exists
            elif (method_info.get_formal_params()[formal] not in self.interpreter.get_classes() and actual.type() == Type.CLASS):
                self.interpreter.error(ErrorType.NAME_ERROR, "no object of type " + method_info.get_formal_params()[formal])
        ...
        if status == ObjectDef.STATUS_RETURN:
            ...
            if (return_value != None):
                if (return_value.type() == Type.NOTHING):
                    if (method_info.get_return_type() == 'int'):
                        return Value(Type.INT, 0)
                    elif (method_info.get_return_type() == 'bool'):
                        return Value(Type.BOOL, False)
                    elif (method_info.get_return_type() == 'string'):
                        return Value(Type.STRING, "")
                    elif (method_info.get_return_type() in self.interpreter.get_classes()):
                        return Value(Type.CLASS, None)
                elif (return_value.type() == Type.INT):
                    if (method_info.get_return_type() != 'int'):
                        self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect return type ")
                elif (return_value.type() == Type.BOOL):
                    if (method_info.get_return_type() != 'bool'):
                        self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect return type ")
                elif (return_value.type() == Type.STRING):
                    if (method_info.get_return_type() != 'string'):
                        self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect return type ")
                elif (return_value.type() == Type.CLASS):
                    if (method_info.get_return_type() not in self.interpreter.get_classes()):
                        self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect return type ")
                elif (method_info.get_return_type() == 'void'):
                    self.interpreter.error(ErrorType.TYPE_ERROR, "incorrect return type ")
            return return_value
        # The method didn't explicitly return a value, so return a value of type nothing
        # if return value is nothing
            # check intended return type, return default value depending on it
        if (method_info.get_return_type() == 'int'):
            return Value(Type.INT, 0)
        elif (method_info.get_return_type() == 'bool'):
            return Value(Type.BOOL, False)
